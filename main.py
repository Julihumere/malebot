...
def main():
    ...
    async def boraaa(ctx):
        ...
        await send_to_malebot_channel(f"Llego el malebot, aguardaaaaaaaaaaa!!!!",
                                      ctx)
    ...
    async def diseloMale(ctx, *arg):
        ...
        if audio is None:
            await send_to_malebot_channel(
                "Todavía el sinverguenza no aprendió a decir eso", ctx)
            return

        ctx.voice_client.stop()
        ctx.voice_client.play(discord.FFmpegPCMAudio(audio['audio']))
        await send_to_malebot_channel(f"El malebot va a decir '{audio['name']}'",
                                      ctx)
    ...
    async def cantaloMale(ctx, url: str):
        ...
        if not url:
            await send_to_malebot_channel(
                "Pone una url de youtube gato, para que el malebot cante", ctx)
        ...
        if len(queueSeconday) > 1:
            await send_to_malebot_channel(
                "El malebot ya guardo el tema en la lista, awanta un toque", ctx)
    ...
    async def laquesiguemale(ctx, primero=False):
        ...
        if len(queue) > 0:
            if not primero:
                await send_to_malebot_channel(
                    'Ahi te pongo el tema que sigue gato', ctx)
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    await send_to_malebot_channel(
                        f"El malebot te va cantar esta: {name_audio}", ctx)

            except Exception as e:
                await send_to_malebot_channel(
                    f"Al malebot no le dío el bocho para cantar eso", ctx)
                return
            ...
            ctx.voice_client.play(discord.FFmpegOpusAudio(url_audio,
                                                          **ffmpeg_options),
                                  after=lambda e: asyncio.run_coroutine_threadsafe(
                                      chequearYReproducir(ctx), bot.loop))
        else:
            await send_to_malebot_channel(
                "El malebot se quedo sin temas para cantar, prendele alguna", ctx)
    ...
    async def chequearYReproducir(ctx):
        ...
        if len(queue) > 0:
            ...
            await send_to_malebot_channel(
                "Ya no hay más canciones en la cola, ¡prendele alguna!", ctx)
